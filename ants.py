...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        spot =0
        while place != beehive:
            if spot>=self.min_range and spot<=self.max_range:
                if place.bees:
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        "*** YOUR CODE HERE ***"
        bees_in_the_spot = self.place.bees[:]
        for bee in bees_in_the_spot:
            bee.reduce_armor(self.damage)
        # END Problem 7
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if not other.is_container and not self.contained_ant :
            return True
        return False
        # END Problem 9
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.imposter:
            ...
            while place:
                if place.ant:
                    if place.ant.has_been_buffed == False:
                        place.ant.has_been_buffed = True
                        place.ant.damage*=2
                    if place.ant.is_container == True and place.ant.contained_ant:
                            if place.ant.contained_ant.has_been_buffed == False:
                                place.ant.contained_ant.has_been_buffed = True
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        Ant.reduce_armor(self,amount)
        if self.armor<=0 and self.imposter == False:
            bees_win()
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    old = bee.action
    new = effect(bee.action,bee)
    def apply(colony):
        nonlocal duration
        if duration == 0:
            old(colony)
            if effect is make_scare:
                bee.has_been_scared = False
                bee.scared = True
